...
{
    const { car_make, car_model, car_year } = inventory.find(
        item => item.id == 33
    );
...
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
{
    ...
    const { car_make, car_model } = inventory[lastCar];

    console.table({ car_make, car_model });
}
...
const carYears = inventory.map(car => car.car_year).sort();
